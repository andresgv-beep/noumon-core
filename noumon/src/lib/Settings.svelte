  ...
  <div class="body">
    ...
    <section class="card">
      <div class="srow">
        <div class="sinfo">
          <b>{t('settings.skin')}</b>
          <small>{t('settings.skinDesc')}</small>
        </div>
      </div>
      <div class="langs">
        {#each SKINS as sk}
          <button class="lang" class:on={theme.skin === sk.code} onclick={() => setSkin(sk.code)}>
            <span class="ticon"><Icon name={sk.icon} size={17} /></span>
            <span class="lname">{t(sk.labelKey)}</span>
            {#if theme.skin === sk.code}<Icon name="check" size={16} />{/if}
          </button>
        {/each}
      </div>
    </section>
    ...
    <section class="card">
      ...
      <div class="swatches" aria-label={t('settings.accent')}>
        {#each ACCENTS as a (a.hex)}
          <button class="swatch" class:on={theme.accent === a.hex} title={t(a.labelKey)}
            style:background={a.hex} onclick={() => setAccent(a.hex)}>
            {#if theme.accent === a.hex}<Icon name="check" size={15} />{/if}
          </button>
        {/each}
        <input class="accent-pick" type="color" value={theme.accent || '#7c6cf0'}
          aria-label={t('settings.accent')}
          oninput={(e) => setAccent(e.currentTarget.value)} />
        <button class="accent-reset" class:on={!theme.accent} onclick={() => setAccent('')}>
...
<style>
  ...
  .card{background:var(--card);border:1px solid var(--border);border-radius:var(--r-lg);padding:18px 18px;box-shadow:var(--shadow)}
  ...
  .field input{height:42px;border-radius:var(--r-md);border:1px solid var(--border);background:var(--ground);color:var(--ink);padding:0 13px;font-size:14px;outline:none;transition:border-color .12s,background .12s}
  ...
  .save{height:38px;padding:0 15px;border-radius:var(--r-md);border:1px solid color-mix(in srgb,var(--accent) 50%,var(--border));background:color-mix(in srgb,var(--accent) 15%,transparent);color:var(--ink);font-size:13px;font-weight:620}
  ...
  .local-server{display:flex;align-items:center;gap:9px;margin-top:16px;padding:12px 14px;border-radius:var(--r-md);background:var(--ground);border:1px solid var(--border);color:var(--ink-dim);font-size:13px}
  ...
  .swatch{width:30px;height:30px;border-radius:var(--r-round);display:grid;place-items:center;color:#fff;border:2px solid transparent;box-shadow:inset 0 0 0 1px rgba(255,255,255,.16);transition:transform .12s,border-color .12s}
  .swatch:hover{transform:translateY(-1px)}
  .swatch.on{border-color:var(--ink)}
  .accent-pick{width:36px;height:30px;padding:0;border:1px solid var(--border);border-radius:var(--r-sm);background:var(--card);cursor:pointer}
  .accent-reset{padding:6px 12px;border:1px solid var(--border);border-radius:var(--r-md);background:var(--card);color:var(--ink-dim);font-size:12.5px}
  .accent-reset:hover{border-color:var(--accent-line);color:var(--ink)}
  .accent-reset.on{border-color:var(--accent-line);color:var(--accent-2)}

  .langs{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;margin-top:16px}
  .lang{display:flex;align-items:center;gap:11px;padding:12px 14px;border-radius:var(--r-lg);border:1px solid var(--border);background:var(--ground);color:var(--ink-dim);text-align:left;box-shadow:var(--shadow);transition:border-color .12s,background .12s,color .12s}
